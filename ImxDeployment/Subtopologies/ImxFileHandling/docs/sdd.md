...
* Uses **instance properties** (IDs, queue sizes, stack sizes, priorities) defined in `ImxFileHandlingConfig` for these static instances (see §4).

### 2.3 Required Inputs for Operation

* **Rate Groups**: Connect scheduler outputs to the **Run** (scheduling) ports of `fileDownlink`.
* **Communication/Framing Stack**: Wire file-packet ports between ImxFileHandling and your COM/framing subtopology (e.g., `ComCcsds`, `ComFprime`, `FramingFprime`, `FramingCcsds`) to complete uplink/downlink paths.
...
```fpp
topology Flight {
  instance ImxFileHandling.Subtopology

  param connections instance ImxFileHandling.prmDb

  # Schedule the active/queued file components (example)
  connections RateGroups {
    rg.RateGroupMemberOut[0] -> ImxFileHandling.Subtopology.fileDownlinkRun
  }

  connections ComCcsds_ImxFileHandling {
    # File Downlink <-> ComQueue
    ImxFileHandling.Subtopology.fileDownlinkBufferSendOut -> ComCcsds.Subtopology.bufferQueueIn[ComCcsds.Ports_ComBufferQueue.FILE]
    ComCcsds.Subtopology.bufferReturnOut[ComCcsds.Ports_ComBufferQueue.FILE] -> ImxFileHandling.Subtopology.fileDownlinkBufferReturn
    
    # Router <-> FileUplink
    ComCcsds.Subtopology.fileUplinkOut                    -> ImxFileHandling.Subtopology.fileUplinkBufferSendIn
    ImxFileHandling.Subtopology.fileUplinkBufferSendOut     -> ComCcsds.Subtopology.fileUplinkReturnIn
  }
```
...
> `Svc/Subtopologies/ImxFileHandling/ImxFileHandlingConfig/ImxFileHandlingConfig.fpp`. The generated constants header for this module (e.g., `FppConstantsAc.hpp`) reflects these settings. ([FPrime][2])

### 4.1 Component properties (`ImxFileHandlingConfig.fpp`)
...
| SVC-FILEHANDLING-005 | `ImxFileHandlingConfig` (instance properties) |
