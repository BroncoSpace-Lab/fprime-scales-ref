# ImxComCcsds (CCSDS Framing) Subtopology — Software Design Document (SDD)

The **ImxComCcsds subtopologies** implement F´’s **CCSDS** communications stack for framing/deframing on the flight side. There are **two variants** in the same module:
...
Both variants provide the standard **router + ComQueue + CCSDS framers/deframers** path and are tuned through **ImxComCcsdsConfig** instance properties.

> [!IMPORTANT]
> `ImxComCcsds` provides framing/deframing for CCSDS SpacePackets inside TM/TC data transfer frames.
...
| SVC-COMCCSDS-006 | Support **configurable instance properties** (IDs, queue sizes, stack sizes, priorities) via `ImxComCcsdsConfig`. | Inspection |
...
Below are **two usage patterns**, one for each variant. Replace identifiers/ports with the **exact names in `ImxComCcsds.fpp`**.

### 3.1 Variant A — ImxComCcsds **with** `Svc::ComStub` (expects a ByteStream driver)

```fpp
topology Flight {
  instance ImxComCcsds.Subtopology

instance comDriver: <ByteStreamDriverInterface>

# (A1) Schedule ComQueue telemetry downlink (optional)
  connections RateGroups {
    rg.RateGroupMemberOut[0] -> ImxComCcsds.Subtopology.comQueueRun
  }

  # (A2) Wire ByteStream driver <-> ComStub supplied by the subtopology
  connections Link {
    comDriver.$recv                                -> ImxComCcsds.Subtopology.drvReceiveIn
    ImxComCcsds.Subtopology.drvReceiveReturnOut       -> comDriver.recvReturnIn
    ImxComCcsds.Subtopology.drvSendOut                -> comDriver.$send
    comDriver.ready                                -> ImxComCcsds.Subtopology.drvConnected
  }
}
```

> [!TIP]
> `ImxComCcsds.commsBufferManager` and `ImxComCcsds.commsBufferSendIn` can be used if the `ByteStreamDriver` requires buffer management.

### 3.2 Variant B — ImxComCcsds **without** `Svc::ComStub`

```fpp
topology Flight {
  import ImxComCcsds.FramingSubtopology
  ...
  connections RateGroups {
    rg.RateGroupMemberOut[0] -> ImxComCcsds.comQueue.run
  }

  # (B3) Wire your ComInterface between the driver and the ImxComCcsds framer/deframer
  connections Link {
    # Downlink: TM framer -> your ComInterface
    ImxComCcsds.framer.dataOut         -> radio.dataIn
    radio.dataReturnOut               -> ImxComCcsds.framer.dataReturnIn
    radio.comStatusOut                -> ImxComCcsds.framer.comStatusIn

    # Uplink: your ComInterface -> frame accumulator
    radio.dataOut                     -> ImxComCcsds.frameAccumulator.dataIn
    ImxComCcsds.frameAccumulator.dataReturnOut -> radio.dataReturnIn
  }
}
```
...
> Configure **only the instance properties** owned by the ImxComCcsds subtopologies. All knobs live under:
> `Svc/Subtopologies/ImxComCcsds/ImxComCcsdsConfig/ImxComCcsdsConfig.fpp`

### 4.1 Component properties (`ImxComCcsdsConfig.fpp`)
...
| SVC-COMCCSDS-006 | `ImxComCcsdsConfig` module                                                                |
