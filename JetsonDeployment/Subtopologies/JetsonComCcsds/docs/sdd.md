# JetsonComCcsds (CCSDS Framing) Subtopology — Software Design Document (SDD)

The **JetsonComCcsds subtopologies** implement F´’s **CCSDS** communications stack for framing/deframing on the flight side. There are **two variants** in the same module:
...
Both variants provide the standard **router + ComQueue + CCSDS framers/deframers** path and are tuned through **JetsonComCcsdsConfig** instance properties.

> [!IMPORTANT]
> `JetsonComCcsds` provides framing/deframing for CCSDS SpacePackets inside TM/TC data transfer frames.
...
| ---------------- | -------------------------------------------------------------------------------------------------------------- | ---------- |
| SVC-JETSONCOMCCSDS-001 | Provide a **CCSDS framer** to convert COM buffers into CCSDS frames for downlink.                             | Inspection |
| SVC-JETSONCOMCCSDS-002 | Provide a **CCSDS deframing path** to parse incoming CCSDS frames into COM buffers for uplink.                 | Inspection |
| SVC-JETSONCOMCCSDS-003 | Provide an F´ **router** to route deframed packets (e.g., commands/files) into the flight software.            | Inspection |
| SVC-JETSONCOMCCSDS-004 | Provide a **subtopology variant that supplies `Svc::ComStub`** designed to connect to a ByteStream driver.     | Inspection |
| SVC-JETSONCOMCCSDS-005 | Provide a **subtopology variant that expects an external `Svc::ComInterface`** supplied by the deployment.     | Inspection |
| SVC-JETSONCOMCCSDS-006 | Support **configurable instance properties** (IDs, queue sizes, stack sizes, priorities) via `JetsonComCcsdsConfig`. | Inspection |
...
Below are **two usage patterns**, one for each variant. Replace identifiers/ports with the **exact names in `JetsonComCcsds.fpp`**.

### 3.1 Variant A — JetsonComCcsds **with** `Svc::ComStub` (expects a ByteStream driver)

```fpp
topology Flight {
  instance JetsonComCcsds.Subtopology

instance comDriver: <ByteStreamDriverInterface>

# (A1) Schedule ComQueue telemetry downlink (optional)
  connections RateGroups {
    rg.RateGroupMemberOut[0] -> JetsonComCcsds.Subtopology.comQueueRun
  }

  # (A2) Wire ByteStream driver <-> ComStub supplied by the subtopology
  connections Link {
    comDriver.$recv                                -> JetsonComCcsds.Subtopology.drvReceiveIn
    JetsonComCcsds.Subtopology.drvReceiveReturnOut       -> comDriver.recvReturnIn
    JetsonComCcsds.Subtopology.drvSendOut                -> comDriver.$send
    comDriver.ready                                -> JetsonComCcsds.Subtopology.drvConnected
  }
}
```

> [!TIP]
> `JetsonComCcsds.commsBufferManager` and `JetsonComCcsds.commsBufferSendIn` can be used if the `ByteStreamDriver` requires buffer management.

### 3.2 Variant B — JetsonComCcsds **without** `Svc::ComStub`

```fpp
topology Flight {
  import JetsonComCcsds.FramingSubtopology
  ...
  connections RateGroups {
    rg.RateGroupMemberOut[0] -> JetsonComCcsds.comQueue.run
  }

  # (B3) Wire your ComInterface between the driver and the JetsonComCcsds framer/deframer
  connections Link {
    # Downlink: TM framer -> your ComInterface
    JetsonComCcsds.framer.dataOut         -> radio.dataIn
    radio.dataReturnOut               -> JetsonComCcsds.framer.dataReturnIn
    radio.comStatusOut                -> JetsonComCcsds.framer.comStatusIn

    # Uplink: your ComInterface -> frame accumulator
    radio.dataOut                     -> JetsonComCcsds.frameAccumulator.dataIn
    JetsonComCcsds.frameAccumulator.dataReturnOut -> radio.dataReturnIn
  }
}
```
...
> Configure **only the instance properties** owned by the JetsonComCcsds subtopologies. All knobs live under:
> `Svc/Subtopologies/JetsonComCcsds/JetsonComCcsdsConfig/JetsonComCcsdsConfig.fpp`

### 4.1 Component properties (`JetsonComCcsdsConfig.fpp`)
...
| ---------------- | -------------------------------------------------------------------------------------- |
| SVC-JETSONCOMCCSDS-001 | `spacePacketFramer` — `Svc.Ccsds.SpacePacketFramer`, `tmFramer` — `Svc.Ccsds.TmFramer` |
| SVC-JETSONCOMCCSDS-002 | `frameAccumulator` — `Svc.FrameAccumulator`                                            |
| SVC-JETSONCOMCCSDS-003 | `fprimeRouter` — `Svc.FprimeRouter`                                                    |
| SVC-JETSONCOMCCSDS-004 | `Subtopology` (variant including `Svc.ComStub`)                                        |
| SVC-JETSONCOMCCSDS-005 | `FramingSubtopology` (variant expecting external `Svc.ComInterface`)                   |
| SVC-JETSONCOMCCSDS-006 | `JetsonComCcsdsConfig` module                                                                |
